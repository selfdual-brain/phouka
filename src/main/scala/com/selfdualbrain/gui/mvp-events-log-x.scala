...
class EventsLogViewX(val guiLayoutConfig: GuiLayoutConfig) extends PlainPanel(guiLayoutConfig) with MvpView[SimulationDisplayModel, EventsLogPresenterX] {
  ...
  private var swingTableModel: EventsLogTableModelX = _
  ...
  override def afterModelConnected(): Unit = {
    swingTableModel = new EventsLogTableModelX(this.model)
...
class EventsLogTableModelX(simulationDisplayModel: SimulationDisplayModel) extends AbstractTableModel {
  ...
  simulationDisplayModel.subscribe(this) {
    ...
    case Ev.SimulationAdvanced(numberOfSteps, lastStep, firstInsertedRow, lastInsertedRow) =>
      if (firstInsertedRow.isDefined) fireTableRowsInserted(firstInsertedRow.get, lastInsertedRow.get)
    case other => //ignore
  }
  ...
  private def eventDetails(event: Event[ValidatorId]): String = event match {
    ...
    case Event.Semantic(id, timepoint, source, payload) =>
      payload match {
        ...
        case OutputEventPayload.EquivocationCatastrophe(validators, absoluteFttExceededBy, relativeFttExceededBy) => s"absolute ftt exceeded by $absoluteFttExceededBy, relative ftt exceeded by $relativeFttExceededBy"
      }
  }
