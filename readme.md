...
and the documentation on the config files layout is not there. You will need to feel comfortable building the project
from sources. You must also feel comfortable starting the app "manually" by invoking the entry point (i.e. a class
with "main" method). Most likely you would be editing some params directly in code, so I pretty much assume
you are a developer using some IDE (like IntelliJ Idea).

The entry point to start playing with is this class:
```
...
Caution: `Demo 1` instructs the engine to use a different random seed on ever run, so each time you start a simulation, you will
...
- DES is implemented in a classic way (priority queue of events), so we do not utilize parallelism. This is for
  ...
  considered in the future (see the section "Future plans").
- The coding style is a proper mix of OO and FP. Partially because I liked it this way, but also in many cases
  this is caused by explicit performance optimizations.
...
- extracting the engine as separate library
- Docker support
...
Phouka is based around "Casper the friendly ghost" line of protocols research, and especially the influencing
paper was [Highway: Efficient Consensus with Flexible Finality](https://arxiv.org/abs/2101.02159). Nevertheless,
we are doing rather creative interpretation of the ideas described, plus we use own naming:

- we use blocks and ballots (instead of "units"); "brick" is a name used to denote a block or a ballot
...
Current model implements only the "cloning" model of equivocators, so when a validator node splits into N identical, independent validator
...
the binary size of messages implementing the simulation. So for example one can configure the header binary size,
validator id binary size and so on.

## Nodes model

As we have the exact simulation of real-time (with microsecond resolution), we can simulate computing power
...
We simulate nodes as if they are sequential computers (1 processor, single core). For every node its
computing power defined in sprockets is configured.

A node can become crashed. After a crash, a node becomes deaf (does not react to incoming messages and also
...
another GPLv3 project. Anyone is kindly encouraged to use Phouka in the context of blockchain research or for
doing blockchain-related presentations. Also, if you want to include Phouka as part of some in-house
project (so, not distributed), you are free to do so.
